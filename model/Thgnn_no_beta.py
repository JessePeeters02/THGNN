...
class GraphAttnSemIndividual(Module):
    ...
    def forward(self, inputs, requires_weight=False):
        # beta = torch.ones(inputs.size(1)) / inputs.size(1) # uniforme gewichten (1/3, 1/3, 1/3)
        beta = torch.zeros(inputs.size(0), inputs.size(1), 1).to(inputs.device)  # Shape: [batch, 3, 1]
        beta[:, 0, :] = 1.0  # Zet alleen 'self' op 1
        if requires_weight:
...
class StockHeteGAT(nn.Module):
    ...
    def forward(self, inputs, pos_adj, neg_adj, requires_weight=True):
        ...
        all_embedding = torch.stack((support, torch.zeros_like(support), torch.zeros_like(support)), dim=1)
        all_embedding, sem_attn_weights = self.sem_gat(all_embedding, requires_weight)
        all_embedding = self.pn(all_embedding)
        if requires_weight:
            batch_size = inputs.size(0)
            dummy_attn_shape = (self.pos_gat.num_heads, batch_size, batch_size)
            pos_attn_weights = torch.zeros(dummy_attn_shape, device=inputs.device)
            neg_attn_weights = torch.zeros(dummy_attn_shape, device=inputs.device)
