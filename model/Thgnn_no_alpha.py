...
class GraphAttnMultiHead(Module):
    ...
    def forward(self, inputs, adj_mat, requires_weight=False):
        ...
        # Vervang attention door uniforme gewichten
        degree = adj_mat.sum(dim=1, keepdim=True)  # Aantal buren per node
        uniform_weights = adj_mat / (degree + 1e-8)  # Gelijk gewicht voor elke buur
        uniform_weights = uniform_weights.unsqueeze(0).expand(self.num_heads, -1, -1)  # Voor multi-head
        
        support = torch.matmul(uniform_weights, support)  # Gemiddelde aggregatie
        ...
        if requires_weight:
            return support, uniform_weights
        else:
            return support, None
