...
def calculate_label(raw_df, current_date):
    ...
    close_tomorrow = raw_df.iloc[i+1]['Close']
    return float(close_tomorrow / close_today - 1.0)
...
def main(args):
    ...
    relation_dir = os.path.join(dataset_dir, "relation_STATIC")                 # log e.d.
    snapshot_dir = os.path.join(dataset_dir, "intermediate_snapshots_STATIC")   # optioneel, hier niet gebruikt
    out_pkl_dir = os.path.join(dataset_dir, "data_train_predict_STATIC")
    out_daily_dir = os.path.join(dataset_dir, "daily_stock_STATIC")
