...
def build_initial_edges_via_cosine_similarity(window_data):
    def gpu_featurewise_cosine(stock_tensor: torch.Tensor):
        ...
        for f in range(n_feat):
            X = stock_tensor[:, f, 1:]     # i(t+1)
            Y = stock_tensor[:, f, :-1]    # j(t)
            X = F.normalize(X, p=2, dim=1)
            Y = F.normalize(Y, p=2, dim=1)
            sims.append(X @ Y.T)           # (n_stocks, n_stocks)
    ...
    for i in range(n_stocks):
        # sterke positieve/negatieve buren van i (inkomend naar i)
        strong_pos = np.where(cos_matrix[i] > sim_threshold_pos)[0]
        strong_neg = np.where(cos_matrix[i] < sim_threshold_neg)[0]  # let op: sim_threshold_neg < 0

        # Gebruik evt. min_neighbors fallback zoals je al deed
        # -- POSITIEF: j -> i
        if len(strong_pos) < min_neighbors:
            top_pos = np.argsort(-cos_matrix[i])[:min_neighbors]
            for j in top_pos:
                if cos_matrix[i, j] > 0:
                    pos_edges.append((j, i))   # *** GERICHT: van j naar i
        else:
            for j in strong_pos:
        ...
        if len(strong_neg) < min_neighbors:
            top_neg = np.argsort(cos_matrix[i])[:min_neighbors]
            for j in top_neg:
                if cos_matrix[i, j] < 0:
                    neg_edges.append((j, i))   # *** GERICHT
        else:
            for j in strong_neg:
    ...
    pos_edges = torch.LongTensor(pos_edges).T if pos_edges else torch.empty((2,0), dtype=torch.long)
    neg_edges = torch.LongTensor(neg_edges).T if neg_edges else torch.empty((2,0), dtype=torch.long)
...
def main1_load():
    ...
    # for date in all_dates[prev_date_num-1:-1]:
        ...
        with torch.no_grad():
            ...
            pos_ssa = snapshot['pos_edges_ssa'].to(device)
            neg_ssa = snapshot['neg_edges_ssa'].to(device)
            embeddings = model(features, pos_ssa, neg_ssa, t)
...
def CSI300():
    ...
    relation_path = os.path.join(data_path, "relation_DSE_gericht")
    os.makedirs(relation_path, exist_ok=True)
    snapshot_path= os.path.join(data_path, "intermediate_snapshots_DSE_gericht")
    os.makedirs(snapshot_path, exist_ok=True)
    data_train_predict_path = os.path.join(data_path, "data_train_predict_DSE_gericht")
    os.makedirs(data_train_predict_path, exist_ok=True)
    daily_stock_path = os.path.join(data_path, "daily_stock_DSE_gericht")
